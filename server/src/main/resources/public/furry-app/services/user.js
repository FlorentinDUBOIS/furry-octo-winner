    ...
    return {
      ...
      getInformations() {
        return $q((resolve, reject) => {
          ...
          $http.get(`${ API }/api/client/${userId}`).then((res) => {
            $log.debug(res.data);
            resolve(res.data);
          }).catch((err) => {
      ...
      getGravatarProfil(email, opts = {}) {
        return $q((resolve, reject) => {
          $http.jsonp(`${gravatarURLGenerator(email, opts)}`, {
          ...
          .then((res) => {
            ...
            resolve(res.data.entry[0]);
          }).catch((err) => {
            console.error(err)
            reject(new Error('Failed to get Gravatar infos'));
          });
        });
      }
    }
    $User.$inject = ['$http', 'jwtHelper', '$q', '$log', 'API']

    function gravatarURLGenerator(email, options) {
        ...
        return base + md5(email) + (options.type? options.type : '.json') + "?" + params.join("&")
      }
  }
} ())
